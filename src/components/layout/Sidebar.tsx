...
export function Sidebar({ collapsed, onToggle, pendingFlags = 0 }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Bottom tab bar — mobile only (<640px), replaces sidebar */}
          ...
          return (
            ...
            >
              <span className={styles.mobileTabIcon}>
                ...
                {badge != null && badge > 0 && <span className={styles.mobileTabDot} />}
              </span>
              <span className={styles.mobileTabLabel}>{label.split(' ')[0]}</span>
            </Link>
          );
        })}
      </nav>
